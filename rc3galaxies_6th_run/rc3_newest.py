...
for line in file("rc3_ra_dec_diameter_pgc.txt"):
    print line
    a = str(line)[0]
    ...
    if (start):
        ...
        for band in bands:
            ...
            montage.mSubimage("SDSS_"+out+".fits","SDSS_"+band+"_"+str(ra)+"_"+str(dec)+"r.fits",ra,dec,2*margin) # mSubImage takes xsize which should be twice the margin (margin measures center to edge of image)
            shutil.move("SDSS_"+band+"_"+str(ra)+"_"+str(dec)+"r.fits",os.getcwd()[:-11] )#if change to :-11 then move out of u,g,r,i,z directory, may be more convenient for mJPEG
            if (DEBUG) : print ("Completed Mosaic for " + band)
            #Writing PGC number into FITS header information
            os.chdir("../..")
            hdulist = pyfits.open("SDSS_"+band+"_"+str(ra)+"_"+str(dec)+"r.fits")
            ...
            hdulist.writeto("SDSS_"+band+"_"+str(ra)+"_"+str(dec)+".fits")
            os.system("rm SDSS_"+band+"_"+str(ra)+"_"+str(dec)+"r.fits")
        # Superimposing R,G,B image mosaics into TIFF using STIFF
        # Image for Viewing purposes
        os.system("stiff "+" SDSS_i_"+str(ra)+"_"+str(dec)+ ".fits "+ " SDSS_r_"+str(ra)+"_"+str(dec)+ ".fits "+" SDSS_g_"+str(ra)+"_"+str(dec)+ ".fits "+ "  -c stiff.conf  " +"  -OUTFILE_NAME  SDSS_"+str(ra)+"_"+str(dec)+"_BEST.tiff    -MAX_TYPE QUANTILE  -MAX_LEVEL 0.99 -COLOUR_SAT  6 -MIN_TYPE QUANTILE -MIN_LEVEL 1  -GAMMA_FAC 0.7 ")
        # Image for emphasizing low-surface sturcture
        os.system("stiff "+" SDSS_i_"+str(ra)+"_"+str(dec)+ ".fits "+ " SDSS_r_"+str(ra)+"_"+str(dec)+ ".fits "+" SDSS_g_"+str(ra)+"_"+str(dec)+ ".fits "+ "  -c stiff.conf  " +"  -OUTFILE_NAME  SDSS_"+str(ra)+"_"+str(dec)+"_LOW.tiff  -MAX_TYPE QUANTILE  -MAX_LEVEL 0.98 -COLOUR_SAT  5  -MIN_TYPE QUANTILE -MIN_LEVEL 0.01  -GAMMA_FAC 0.9  ")  
