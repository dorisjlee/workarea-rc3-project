...
ra= 7.3125          
dec=2.86527777778
radius =  0.0418647739422/2. #radius = diameter/2
margin =1.2* radius
...
for ele in bands:
    ...
    montage.mHdr(str(ra)+" "+str(dec),1.2*(radius *2),out+".hdr")#radius
    ...
    montage.mSubimage("SDSS_"+out+".fits","SDSS_"+ele+"_"+str(trunc(ra))+"_"+str(trunc(dec))+".fits",ra,dec,1.2*(radius *2)) # mSubImage takes xsize which should be twice the margin (margin measures center to edge of image)
...
os.system("stiff "+" SDSS_i_"+str(trunc(ra))+"_"+str(trunc(dec))+ ".fits "+ " SDSS_r_"+str(trunc(ra))+"_"+str(trunc(dec))+ ".fits "+" SDSS_g_"+str(trunc(ra))+"_"+str(trunc(dec))+ ".fits "+ "  -c stiff.conf  " +"  -OUTFILE_NAME  MARGIN_1.2*rad_deg_"+str(trunc(ra))+"_"+str(trunc(dec))+"_COLORSAT_2_MAX_MAN_2.tiff   -COLOUR_SAT  2  -MAX_TYPE MANUAL -MAX_LEVEL 2")
# for b in bands:
# 	os.system("rm -r "+b+"/")
