...
for ele in bands:
    ...
    montage.mSubimage("SDSS_"+out+".fits","SDSS_"+ele+"_"+str(trunc(ra))+"_"+str(trunc(dec))+".fits",ra,dec,0.05) # mSubImage takes xsize which should be twice the margin (margin measures center to edge of image)
...
os.system("stiff "+" SDSS_i_"+str(trunc(ra))+"_"+str(trunc(dec))+ ".fits "+ " SDSS_r_"+str(trunc(ra))+"_"+str(trunc(dec))+ ".fits "+" SDSS_g_"+str(trunc(ra))+"_"+str(trunc(dec))+ ".fits "+ "  -c stiff.conf  " +"  -OUTFILE_NAME  NO_mSubImage_MARGIN_0_1deg_"+str(trunc(ra))+"_"+str(trunc(dec))+"_COLORSAT_2_MAX_MAN_3.tiff   -COLOUR_SAT  2  -MAX_TYPE MANUAL -MAX_LEVEL 3")
# for b in bands:
# 	os.system("rm -r "+b+"/")
